...
export const Footer: React.FC = () => {
  ...
  <div className="max-w-[1320px] mx-auto px-4 md:px-gutter pt-16 md:pt-space-xl pb-8 md:pb-space-lg">
    <div className="bg-surface-container-low rounded-xl p-6 md:p-space-lg lg:p-space-xl mb-12 md:mb-space-xl flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-space-lg">
      <div className="flex flex-col gap-space-xs text-center lg:text-left"><span className="font-label-sm text-label-sm uppercase tracking-widest text-primary">{clientDetails.tagline1}</span>
        <h3 className="font-headline-md text-headline-md text-on-surface font-semibold">{clientDetails.tagline2}</h3>
        <p className="font-body-sm text-body-sm text-on-surface-variant">Reserve your private booth or order gourmet
          favorites directly to your doorstep in {clientDetails.displayLocation}.</p>
      </div>
      <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-space-md w-full lg:w-auto">
        <a className="w-full sm:w-auto text-center px-8 py-4 rounded-full uppercase tracking-wider text-xs sm:text-sm font-semibold bg-primary text-background hover:bg-primary-fixed transition-all" href="#" onClick={(e) => { e.preventDefault(); setIsReservationOpen(true); }}>Reserve Table</a>
        <a className="w-full sm:w-auto text-center px-8 py-4 rounded-full uppercase tracking-wider text-xs sm:text-sm font-semibold border border-primary/30 text-on-surface hover:border-primary hover:bg-primary/5 transition-all" href="#" onClick={(e) => handleDummy(e, 'Online Ordering', 'Our delivery partners Swiggy and Zomato will be integrated soon.')}>Order Online</a>
      </div>
    </div>
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-space-xl mb-12 md:mb-space-xl">
      <div className="col-span-2 lg:col-span-1 flex flex-col gap-space-md">
        <div className="flex items-center gap-space-sm">
          <div className="w-8 h-8 rounded-lg bg-surface-container flex items-center justify-center overflow-hidden drop-shadow-[0_0_10px_rgba(212,175,55,0.1)]">
            <img src="/logo.webp" alt={`${clientDetails.businessName} Logo`} className="w-full h-full object-contain" />
          </div>
          <span className="font-headline-sm text-headline-sm text-on-surface font-semibold">{clientDetails.businessName} {clientDetails.shortName}</span>
        </div>
        <p className="font-body-sm text-body-sm text-on-surface-variant">{clientDetails.description}</p>
        <div className="flex items-center gap-space-sm pt-space-xs">
          <a className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-on-surface-variant hover:border-primary/40 hover:text-primary transition-all" href={clientDetails.instagramLink} target="_blank" rel="noopener noreferrer"><IconInstagram /></a>
          <a className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-on-surface-variant hover:border-primary/40 hover:text-primary transition-all" href={clientDetails.facebookLink} target="_blank" rel="noopener noreferrer"><IconFacebook /></a>
          <a className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-on-surface-variant hover:border-primary/40 hover:text-primary transition-all" href="#" onClick={(e) => handleDummy(e, 'Legal / Info', 'Detailed information regarding this policy will be uploaded before the official launch. Please contact management for immediate queries.')}><IconGoogle /></a>
          <a className="w-10 h-10 rounded-full border border-white/10 bg-white/5 flex items-center justify-center text-on-surface-variant hover:border-primary/40 hover:text-primary transition-all" href={clientDetails.zomatoLink} target="_blank" rel="noopener noreferrer"><IconZomato /></a>
        </div>
      </div>
      <div className="col-span-1 flex flex-col gap-space-md">
        <h4 className="font-label-lg text-label-lg text-on-surface font-semibold uppercase tracking-wider">Cuisine &amp;
          Curations</h4>
        <div className="flex flex-col gap-space-xs font-body-sm text-body-sm"><a className="hover:text-primary transition-colors" href="#" onClick={(e) => handleNav(e, 'menu-categories')}>Artisanal Espresso &amp;
            Brews</a><a className="hover:text-primary transition-colors" href="#" onClick={(e) => handleNav(e, 'chef-specials')}>Chef's
            Signature Platters</a><a className="hover:text-primary transition-colors" href="#" onClick={(e) => handleNav(e, 'menu-categories')}>Wood-Fired Pizzas &amp; Pastas</a><a className="hover:text-primary transition-colors" href="#" onClick={(e) => handleNav(e, 'menu-categories')}>Lounge Mocktails &amp; Shakes</a><a className="hover:text-primary transition-colors" href="#" onClick={(e) => handleNav(e, 'visual-gallery')}>Ambience &amp; Private
            Dining</a></div>
      </div>
      <div className="col-span-1 flex flex-col gap-space-md">
      ...
      <div className="col-span-2 lg:col-span-1 flex flex-col gap-space-md">
        ...
        <div className="flex flex-col gap-space-xs font-body-sm text-body-sm">
          <div className="flex items-start gap-space-xs">
            <span className="material-symbols-outlined text-primary text-sm mt-1 shrink-0">location_on</span>
            <a 
              href={clientDetails.googleMapsLink} 
              target="_blank" 
              rel="noopener noreferrer"
              className="text-on-surface-variant hover:text-primary transition-colors group flex flex-col items-start"
            >
              <span>{clientDetails.address}</span>
              <span className="inline-flex items-center gap-1 font-label-md text-[10px] uppercase tracking-wider text-primary mt-2 opacity-80 group-hover:opacity-100 border border-primary/30 px-3 py-1.5 rounded-full">
          ...
          <div className="flex items-center gap-space-xs pt-space-xs">
            ...
            <a className="hover:text-primary transition-colors" href={`tel:${clientDetails.phone}`}>{clientDetails.displayPhone}</a>
          </div>
          <div className="flex items-center gap-space-xs pt-1">
            ...
            <a className="hover:text-primary transition-colors" href={`https://wa.me/${clientDetails.whatsapp}`} target="_blank" rel="noopener noreferrer">{clientDetails.displayPhone} (WhatsApp)</a>
          </div>
          <div className="flex items-center gap-space-xs"><span className="material-symbols-outlined text-primary text-sm">mail</span><span className="text-on-surface-variant">{clientDetails.email}</span></div>
        </div>
      </div>
    </div>
